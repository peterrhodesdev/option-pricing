...
function price(option) {
  const [ S, K, t, vol, r, q ] = [ option.initialSpotPrice, option.strikePrice, option.timeToMaturity, option.volatility, option.riskFreeRate, option.dividendYield ];
...
function blackScholesMerton(type, S, K, t, vol, r, q) {
  ...
  const d1 = (Math.log(S / K) + (r - q + Math.pow(vol, 2) / 2) * t) / (vol * Math.sqrt(t));
  const d2 = (Math.log(S / K) + (r - q - Math.pow(vol, 2) / 2) * t) / (vol * Math.sqrt(t));

  return isCall * S * Math.exp(-q * t) * standardNormalCdf(isCall * d1)
